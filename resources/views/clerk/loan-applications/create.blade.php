...
<div class="card card-primary">
    ...
    <form action="{{ route('clerk.loan-applications.store') }}" method="POST" data-validate>@csrf
        <div class="card-body">
            <div class="row">
            ...
            <div class="row">
                <div class="col-md-6"><div class="form-group"><label>Guarantor 1</label>
                    <select name="guarantor1_id" class="form-control select2"><option value="">-- None --</option>@foreach($customers as $c)<option value="{{ $c->id }}">{{ $c->full_name }}</option>@endforeach</select></div></div>
                <div class="col-md-6"><div class="form-group"><label>Guarantor 2</label>
                    <select name="guarantor2_id" class="form-control select2"><option value="">-- None --</option>@foreach($customers as $c)<option value="{{ $c->id }}">{{ $c->full_name }}</option>@endforeach</select></div></div>
            </div>
        </div>
