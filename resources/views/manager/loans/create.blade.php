...
<div class="card card-primary">
    ...
    <form action="{{ route('manager.loans.store') }}" method="POST" data-validate>@csrf
        <div class="card-body">
            <div class="row">
                <div class="col-md-6"><div class="form-group"><label>Approved Application</label>
                    <select name="loan_application_id" class="form-control select2"><option value="">-- Select --</option>@foreach($loanApplications as $app)<option value="{{ $app->id }}">{{ $app->customer?->full_name }} - {{ number_format($app->approved_amount ?? $app->applied_amount, 2) }}</option>@endforeach</select></div></div>
                <div class="col-md-6"><div class="form-group"><label>Loan Type <span class="text-danger">*</span></label>
                    <select name="loan_type_id" class="form-control select2" required><option value="">-- Select --</option>@foreach($loanTypes as $t)<option value="{{ $t->id }}">{{ $t->name }} ({{ $t->interest_rate }}%)</option>@endforeach</select></div></div>
            </div>
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="col-md-4"><div class="form-group"><label>Frequency <span class="text-danger">*</span></label>
                    <select name="frequency" class="form-control select2" required><option value="MONTHLY">Monthly</option><option value="WEEKLY">Weekly</option><option value="DAILY">Daily</option></select></div></div>
            </div>
        </div>
