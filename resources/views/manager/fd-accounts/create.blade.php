...
<div class="card card-primary">
    ...
    <form action="{{ route('manager.fd-accounts.store') }}" method="POST" data-validate>@csrf
        <div class="card-body">
            <div class="row">
                <div class="col-md-6"><div class="form-group"><label>Customer <span class="text-danger">*</span></label>
                    <select name="customer_id" class="form-control select2" required><option value="">-- Select --</option>@foreach($customers as $c)<option value="{{ $c->id }}">{{ $c->full_name }} ({{ $c->customer_number }})</option>@endforeach</select></div></div>
                <div class="col-md-6"><div class="form-group"><label>FD Scheme <span class="text-danger">*</span></label>
                    <select name="fd_setup_id" class="form-control select2" required><option value="">-- Select --</option>@foreach($fdSetups as $s)<option value="{{ $s->id }}">{{ $s->description }} ({{ $s->interest_rate }}% / {{ $s->duration_days }} days)</option>@endforeach</select></div></div>
            </div>
            <div class="row">
                ...
                <div class="col-md-4"><div class="form-group"><label>Transaction Mode <span class="text-danger">*</span></label>
                    <select name="transaction_mode" class="form-control select2" required><option value="cash">Cash</option><option value="cheque">Cheque</option></select></div></div>
            </div>
        </div>
